...
def test_compute_uas_las_listcomp():
    ...
    dtree_true = [DT_TRUE]
    # compare gold with itself: perfect scores
    assert compute_uas_las_listcomp(dtree_true, dtree_true) == (1.0, 1.0, 1.0)
    # one error on head
    dtree_pred = [DT_PRED_HD_ERR]
    assert compute_uas_las_listcomp(dtree_true, dtree_pred) == (0.75, 0.75, 1.0)
    # one error on label
    dtree_pred = [DT_PRED_LB_ERR]
    assert compute_uas_las_listcomp(dtree_true, dtree_pred) == (1.0, 0.75, 0.75)
    # one error on both
    dtree_pred = [DT_PRED_BO_ERR]
    assert compute_uas_las_listcomp(dtree_true, dtree_pred) == (0.75, 0.5, 0.75)
